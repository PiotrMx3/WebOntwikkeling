  ...
  const sortTypeFields: Field[] = [
    {
      value: "name",
      text: "Name",
      selected: sortField === "name" ? "selected" : "",
    },
    {
      value: "age",
      text: "Age",
      selected: sortField === "age" ? "selected" : "",
    },
    {
      value: "random",
      text: "Random",
      selected: sortField === "random" ? "selected" : "",
    },
  ];
...
app.get("/", (req, res) => {
  if (typeof req.query.q === "string") {
    ...
    let filteredPersons = persons.filter((person) =>
      person.name.toLowerCase().startsWith(q.toLowerCase().trim())
    );
